...
export default function DashboardKpiRow() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [review, email, activeCount, studentCount] = await Promise.all([
        countFor({ roles: studentRoles, status: "PENDING_ADMIN_REVIEW" }),
        countFor({ roles: studentRoles, status: "PENDING_EMAIL_VERIFICATION" }),
        countFor({ roles: studentRoles, status: "ACTIVE" }),
        countFor({ roles: studentRoles }),
      ]);
      if (!cancelled) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 md:grid-cols-4 mb-10">
      <KpiTile label="Pending student review" value={pendingReview} loading={loading} />
      <KpiTile label="Pending email" value={pendingEmail} loading={loading} />
      <KpiTile label="Active students" value={active} loading={loading} />
      <KpiTile label="Total students" value={students} loading={loading} />
    </div>
  );
}
