...
import type { MembershipPackage } from "@/lib/membership/packages";
import {
...
interface RegistrationPackagesPreviewProps {
  variant: MembershipPackageVariant;
}

const COPY: Record<
  MembershipPackageVariant,
...
> = {
  ...
  service: {
    title: "Membership Plans",
    subtitle:
      "We offer a 3-month free trial for all new photographers and beauticians. After the trial, continue with full access through our affordable annual plan.",
  },
};

export default function RegistrationPackagesPreview({
  variant,
}: RegistrationPackagesPreviewProps) {
  ...
  useEffect(() => {
    fetchPackages().then((pkgs) => {
      setPackages(pkgs.length > 0 ? pkgs : getDefaultPackages(variant));
      setOpen(true);
    });
  }, [variant]);
    ...
    >
      ...
      >
        ...
        <div className="px-6 pt-8 pb-4 text-center border-b border-[#E0E0E0]">
          ...
          <p className="font-ui text-xs text-[#6B6B6B] mt-2 max-w-lg mx-auto leading-relaxed">
            {copy.subtitle}
          </p>
        </div>

        <div className={`grid gap-4 ${gridCols} p-6`}>
          {packages.map((pkg) => (
            <div
              key={pkg.id}
              className={[
            ...
            >
              ...
              <p className="font-ui text-[9px] tracking-[0.1em] uppercase text-[#9A9A9A]">
                {formatPackageDuration(pkg.durationMonths)}
              </p>
