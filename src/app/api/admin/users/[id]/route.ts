...
export async function GET(_request: NextRequest, context: RouteContext) {
  ...
  const { status, data } = await backendFetch(`/v1/users/${id}`, { token });

  if (status !== 200) {
    return NextResponse.json(
      data ?? { message: errorMessage(data, "Failed to fetch user") },
      { status },
    );
  }
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  const { status, data } = await backendFetch(`/v1/users/status/${id}`, {
    method: "PATCH",
    token,
